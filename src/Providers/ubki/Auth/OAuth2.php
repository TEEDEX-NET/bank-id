...
namespace BankID\Providers\ubki\Auth;
...
class OAuth2 implements IOAuth2
{
	...
	public function getOAuth2Url(): string
	{
		...
		$url = "https://$this->url/DataAccessService/das/authorize?$query";
	...
	public function getMaskedAuthorizationHeaders(): array
	{
		...
		return [
			"Authorization" => "Bearer $masked_access_token, Id $client_id",
		];
	}
	...
	private function getAccessToken(): string
	{
		if (!$this->access_token) {
			...
			$body = [
				...
				"client_id" => $this->client_id,
				"client_secret" => $this->getSecretHash(),
				"redirect_uri" => $this->redirect_uri,
				"code" => $this->code,
			];
			$error = null;
			...
			try {
				$url = "https://$this->url/DataAccessService/oauth/token";
	...
	{
		return sha1($this->client_id . $this->client_secret . $this->code);
	}
